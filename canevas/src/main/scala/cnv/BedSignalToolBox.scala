...
object BedSignalToolBox {
    ...
    object BedSpan {
        def length(span: BedSpan) = {
            span.stop - span.start
        }
    }
    case class BedEntry[T](span: BedSpan, value: T)
    object BedEntry {
        def length[T](entry: BedEntry[T]) = {
            BedSpan.length(entry.span)
        }
    }
    ...
    /** Alternative */
    def appendEntryIfJumpRatio(e: Seq[BedEntry[Int]], acc: List[BedEntry[Int]], threhsoldRatio: Double, threhsold: Int = THRESHOLD) = {
        e match {
            case Seq(e1, e2) => {
                // The entries must be contiguous (this test may be removed if the entries are dense)
                if ((e1.span.chr == e2.span.chr) && (e1.span.stop == e2.span.start)) {
                    val sub = e2.value - e1.value
                    val max = math.max(e2.value, e1.value)
                    if ((Math.abs(sub) >= threhsoldRatio * max) && (Math.abs(sub) >= threhsold)) { // Only append if difference is at least threshold times max coverage (e.g., at least 0.3 x max coverage)
                        // This is better if coverage gets really high, e.g., in the hundreds or thousands because of duplications, the jump condition will be relative to this coverage, lowering false positives compared to using a constant threshold.
                        val span = BedSpan(e2.span.chr, e2.span.start, e2.span.start + 1)
                        BedEntry[Int](span, sub) :: acc
                    } else {
                        acc
    ...
    /** Alternative, relative to coverage */
    def generateJumpsFromFile(file: String, thresholdRatio: Double, threshold: Int) = {
        (readBedEntriesFromFile(file, intBedEntryFromLine) sliding 2).foldLeft(List(): List[BedEntry[Int]])((acc, e) => appendEntryIfJumpRatio(e, acc, thresholdRatio, threshold)).reverse
    }
    ...
    /** Extract jumps from mosdepth coverage count (per-base) - This does not work for bedgraph (can be adapted) and does not generate compressed output - old */
    @deprecated
